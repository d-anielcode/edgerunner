"""Does the optimal spread-vs-concentrated strategy change with bankroll size?"""
import duckdb
con = duckdb.connect()
...
def run(start, ks, max_bet, max_daily):
    br = start; peak = start; max_dd = 0.0
    ...
    for _, row in all_trades.iterrows():
        ...
        p2 = pp(sp, yp)
        if p2 is not None:
            ay = p2; km = 0.375 * ks; mp_ = 0.18 * ks; me = 0.05 if sp == "NHL" else 0.08
        else:
            params = SP.get(sp)
            ...
            km = params[0] * 1.5 * ks; mp_ = params[1] * 1.5 * ks; me = params[2]
        edge = (yp / 100.0) - ay
        if edge < me: continue
        nc = np_ / 100.0
        fee = 0.07 * nc * (1 - nc)
    ...
    wr = tw / max(tt, 1) * 100
    roi = (br - start) / start * 100
    return {"final": br, "trades": tt, "wr": wr, "max_dd": max_dd * 100, "roi": roi}


print("=" * 100)
print("DOES OPTIMAL STRATEGY CHANGE WITH BANKROLL SIZE?")
print("=" * 100)

for start_br in [100, 500, 1000, 5000, 10000, 100000]:
    print(f"\n  Starting: ${start_br:>,}")

    # Scale max_bet proportionally — $200 at $100 is 200% which is absurd at $100K
    # Use 3% of starting bankroll as max_bet, min $50, max $5000
    proportional_cap = max(50, min(int(start_br * 0.03), 5000))

    results = []
    for label, ks, md in [
        ("Concentrated (5/day)", 1.0, 5),
        ("Moderate (10/day)", 0.67, 10),
        ("Spread (15/day)", 0.50, 15),
        ("Very spread (25/day)", 0.33, 25),
    ]:
        r = run(start_br, ks, proportional_cap, md)
        results.append((label, r))

    # Find best
    best = max(results, key=lambda x: x[1]["final"])
    best_risk = min(results, key=lambda x: x[1]["max_dd"])

    print(f"  Cap: ${proportional_cap}/trade")
    print(f"  {'Strategy':30s} | {'Final':>14s} | {'ROI':>8s} | {'Trades':>6s} | {'MaxDD':>6s} | Note")
    print(f"  " + "-" * 85)
    for label, r in results:
        note = ""
        if label == best[0]: note = "<-- MOST PROFIT"
        if label == best_risk[0] and label != best[0]: note = "<-- LOWEST RISK"
        if r["final"] < 1e9:
            fs = f"${r['final']:>12,.2f}"
        else:
            fs = f"${r['final']:>12.2e}"
        print(f"  {label:30s} | {fs} | {r['roi']:>+7.0f}% | {r['trades']:>6d} | {r['max_dd']:>5.1f}% | {note}")
