"""Compare conservative vs aggressive sizing."""
import duckdb
con = duckdb.connect()
...
def run(kelly_scale, max_bet):
    ...
    for _, row in all_trades.iterrows():
        ...
        p2 = pp(sp, yp)
        if p2 is not None:
            ay = p2; km = 0.25 * kelly_scale; mp_ = min(0.12 * kelly_scale, 0.30); me = 0.05 if sp == "NHL" else 0.08
        else:
            s = SP.get(sp)
            if not s: continue
            ...
            km = s[0] * kelly_scale; mp_ = min(s[1] * kelly_scale, 0.30); me = s[2]
    ...
    wr = tw / max(tt, 1) * 100
    return {"final": br, "trades": tt, "wr": wr, "max_dd": max_dd * 100, "min_br": min_br}

print("=" * 100)
print("AGGRESSIVE vs CONSERVATIVE SIZING")
print("$100 start, 13 months, all 12 sports")
print("=" * 100)
print()
print(f"{'Config':50s} | {'Final':>12s} | {'Trades':>6s} | {'WR':>5s} | {'MaxDD':>6s} | {'Lowest':>8s}")
print("-" * 100)

configs = [
    (0.5, 50, "Very safe: 0.5x Kelly, $50 cap"),
    (1.0, 50, "Safe: current Kelly, $50 cap"),
    (1.0, 100, "CURRENT: current Kelly, $100 cap"),
    (1.0, 200, "Higher cap only: current Kelly, $200 cap"),
    (1.0, 500, "No real cap: current Kelly, $500 cap"),
    (1.5, 100, "More aggro Kelly: 1.5x Kelly, $100 cap"),
    (1.5, 200, "Aggro: 1.5x Kelly, $200 cap"),
    (2.0, 100, "Double Kelly: 2x Kelly, $100 cap"),
    (2.0, 200, "Very aggro: 2x Kelly, $200 cap"),
    (2.0, 500, "Max aggro: 2x Kelly, $500 cap"),
    (3.0, 200, "YOLO: 3x Kelly, $200 cap"),
]

for scale, cap, label in configs:
    r = run(scale, cap)
    if r["final"] < 1e9:
        final_s = f"${r['final']:>10,.2f}"
    else:
        final_s = f"${r['final']:>10.2e}"
    marker = " <<<" if label.startswith("CURRENT") else ""
    print(f"{label:50s} | {final_s} | {r['trades']:>6d} | {r['wr']:>4.1f}% | {r['max_dd']:>5.1f}% | ${r['min_br']:>7.2f}{marker}")

print()
print("THE TRADEOFF:")
print("  More profit requires tolerating bigger drawdowns.")
print("  At 2x Kelly + $200 cap, you could see bankroll drop 80%+ before recovering.")
print("  At current settings, worst drawdown is ~50-75% but final balance is lower.")
